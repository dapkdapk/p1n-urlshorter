    ...
    $.post(scriptLocation(), data_to_send, "json")
        ...
        .success(function (data) {
            if (data.status == 0) {
                ...
                v.qrButton(false);
                v.qrNewCodeSpan(false);
                document.getElementById("qrcodenew").innerHTML = "";
...
var clickDownloadShortUrl = function(){
    const _url = v.shortUrlString().split('/');
    var _code = _url[_url.length - 1];
    qrCode.download({ name: ("qrcode_"+location.hostname).replace(/\./gi, "")+"_"+_code, extension: "png" });
}
...
var clickGetQRCode  = function () {
    ...
    qrCode = new QRCodeStyling({
        ...
        type: "svg",
        image: scriptLocation() + "icon.php",
        //image: scriptLocation() + "svg.php?t=" + window.location.hostname,
        dotsOptions: {
            color: "#000",
            type: "rounded"
        },
